...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		healthBarBG = new AttachedSprite('heal-bar');
		healthBarBG.y = 550;
		healthBarBG.screenCenter(X);
		healthBarBG.scale.x = 1;
		healthBarBG.scale.y = 1;
		healthBarBG.scrollFactor.set();
		healthBarBG.visible = !ClientPrefs.hideHud;
		healthBarBG.xAdd = -4;
		healthBarBG.yAdd = -4;
		
		PlayerBar = new AttachedSprite('BarColores');
		PlayerBar.y = 575;
		PlayerBar.screenCenter(X);
		PlayerBar.scale.x = 1;
		PlayerBar.scale.y = 1;
		PlayerBar.scrollFactor.set();
		PlayerBar.visible = !ClientPrefs.hideHud;

		OpponentBar = new AttachedSprite('BarColores');
		OpponentBar.y = 575;
		OpponentBar.screenCenter(X);
		OpponentBar.scale.x = 1;
		OpponentBar.scale.y = 1;
		OpponentBar.scrollFactor.set();
		OpponentBar.visible = !ClientPrefs.hideHud;

		if(ClientPrefs.downScroll) healthBarBG.y = 15;

		healthBar = new FlxBar(healthBarBG.x, healthBarBG.y + 65, RIGHT_TO_LEFT, Std.int(healthBarBG.width * 0.85), Std.int(healthBarBG.height * 0.45) , this,
			'health', 0, 2);
		...
		scoreTxt = new FlxText(0, 685, FlxG.width, "", 20);
		scoreTxt.setFormat(Paths.font("vcr.ttf"), 20, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		scoreTxt.scrollFactor.set();
		scoreTxt.borderSize = 1.25;
		scoreTxt.visible = !ClientPrefs.hideHud;
		if(ClientPrefs.downScroll) scoreTxt.y = 100;
		add(scoreTxt);
	...
	public function reloadHealthBarColors() {
		PlayerBar.color = FlxColor.fromRGB(boyfriend.healthColorArray[0], boyfriend.healthColorArray[1], boyfriend.healthColorArray[2]);
		OpponentBar.color = FlxColor.fromRGB(dad.healthColorArray[0], dad.healthColorArray[1], dad.healthColorArray[2]);
	...
	override public function update(elapsed:Float)
	{
		...
		var Longest = new FlxRect((healthBar.width * (FlxMath.remapToRange(healthBar.percent, 0, 100, 100, 0) * 0.01)) + 22, 0, PlayerBar.frameWidth, PlayerBar.frameHeight);
		PlayerBar.clipRect = Longest;
