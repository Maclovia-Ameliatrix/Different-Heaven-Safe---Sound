...
class TitleState extends MusicBeatState
{
	...
	function startIntro()
	{
		...
		bg.loadGraphic(Paths.image('cielo'));
		...
		titleText = new FlxSprite(350, 550);
		...
		if (initialized)
			...
			initialized = true;
				...
				FlxTween.tween(logoBl, {y: -100}, 3, {
					ease: FlxEase.cubeInOut,
					type: PINGPONG
				});

				FlxTween.tween(logoBl, {angle: -2}, 7, {
					ease: FlxEase.cubeInOut,
					type: PINGPONG
				});
	...
	override function update(elapsed:Float)
	{
		...
		if (initialized && !transitioning && skippedIntro)
		{
			...
			if(pressedEnter)
			{
				...
				FlxG.camera.flash(0x6EFFFFFF, 0.75);
